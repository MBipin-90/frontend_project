...
import NabBarStudent from '../NabBarStudent/NabBarStudent';
import styles from './StudentUser.module.css';

const StudentUser = () => {
    const location = useLocation();
    const studentInfo = location.state || {}; // Retrieve faculty info from state
    ...
    useEffect(() => {
        const fetchStudents = async () => {
            try {
                const response = await fetch(`http://localhost:5000/students?internshipDomain=${studentInfo.internshipDomain}`);
        ...
        const fetchTasks = async () => {
            try {
                const response = await fetch(`http://localhost:5000/tasks?facultyId=${studentInfo.id}`);
        ...
        fetchStudents();
        fetchTasks();
    }, [studentInfo.internshipDomain, studentInfo.id]);
    ...
    const handleTaskFormSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost:5000/tasks', {
                ...
                body: JSON.stringify({
                    internshipDomain: studentInfo.internshipDomain,
            ...
            const updatedResponse = await fetch(`http://localhost:5000/tasks?facultyId=${studentInfo.id}`);
    ...
    return (
        <div>
            <NabBarStudent studentInfo={studentInfo} />
            <div className={styles.mainContainer}>
                {/* Left Section: Students */}
                <div className={styles.leftSection}>
                    <h2 className={styles.sectionTitle}>{studentInfo.internshipDomain || "Unknown"} Students</h2>
